# 🚀 FlexJobs Jobs Scraper – Scrape FlexJobs Jobs to JSON, CSV & Excel

Scrape and aggregate job listings from **FlexJobs** (flexjobs.com) search results – job title, company, location, salary, posting date and a direct apply link – and export them to JSON, CSV, Excel or an API in a single run.

[View on Apify Store](#)

## 📖 Overview

The **FlexJobs Jobs Scraper** is a data-extraction tool that turns FlexJobs job search results into structured data. FlexJobs is a curated job board for remote, hybrid and flexible-schedule roles.
...
- **🔍 Full-Text Job Search** – Search FlexJobs by keyword, job title or skill and walk every result page automatically.
...
```
https://mcp.apify.com/?tools=actors,docs,orgupdate/flexjobs-jobs-scraper
```
...
With the server connected, your agent can search FlexJobs listings by title and city, compare postings across employers, and pull salary and apply links into a single structured answer.
...
| `companyName` | String | No | Restrict results to a single employer (e.g. `"Dell"`). | - | Any company name |
...
Not every FlexJobs listing includes every field – `salary`, `job_type` and exact `date` vary by posting – so handle optional fields gracefully in your integration.
...
Run this Actor from the Apify API with the official clients. Replace `orgupdate/flexjobs-jobs-scraper` with the real Actor ID from the Actor page.
...
run = client.actor("orgupdate/flexjobs-jobs-scraper").call(run_input=run_input)
...
const run = await client.actor('orgupdate/flexjobs-jobs-scraper').call(input);
...
- **Find remote FlexJobs jobs via MCP** – let an AI agent pull remote listings with company, source and direct apply links.
- **Track Dell's open roles** – monitor one employer's postings on FlexJobs on a daily schedule.
- **Jobs posted in the last 3 days** – recent FlexJobs listings filtered by `datePosted`, feeding a database.
- **Salary benchmark by city** – aggregate FlexJobs pay ranges for one job title across multiple locations.

## 💡 Use Cases

- **Job boards & aggregators** – Populate a niche board with fresh FlexJobs listings, no manual entry.
...
Some queries genuinely have few listings on FlexJobs. Broaden the keyword (e.g. `"engineer"` instead of `"senior backend engineer"`), widen `datePosted`, and raise `pagesToFetch`.
