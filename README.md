...
The most efficient way to extract job listings directly from Flexjobs Jobs search results.

[View on Apify Store](#)

## 📖 Overview

The **Flexjobs Jobs Scraper** is a powerful data extraction tool designed to aggregate job listings from the Flexjobs Jobs search engine. Since Flexjobs aggregates listings from Glassdoor, LinkedIn, ZipRecruiter, and direct company career pages, this actor serves as a "One-Stop Shop" for global employment data.
...
- **🌍 Multi-Source Aggregation** – Access listings from thousands of job boards via a single Flexjobs search query.
...
|-----------|------|----------|-------------|---------|--------------|
| `countryName` | String | No | The country context for the Flexjobs Search domain | "all" | "usa", "uk", "india", etc. |
...
| `companyName` | String | No | Filter listings to a specific employer | - | Any company name (e.g., "Flexjobs", "Microsoft") |
...
- **posted_via:** The original platform where Flexjobs found the job (e.g., Monster, Greenhouse, LinkedIn).
...
- **Flexjobs Sheets:** Automatically save new job listings into a spreadsheet for analysis.
